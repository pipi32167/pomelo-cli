...
consts.COMANDS_ALL = [
	...
	["runAll", " run script in all servers"],
	["onlineUser", " show online user count in gamelogic server"],
	["user", " show user count in gamelogic server"],
	["heapDiff", " debug command, print heap diff to log"],
	["msgCount", " debug command, print message handle count"],
	["msgTime", " debug command, print message cost time"],
	["gc", " debug command, force gc, run process with option --expose-gc"],
];
...
consts.COMANDS_MAP = {
	...
	"runAll": ["\nrun script in all servers",
			"example: runAll app.get(\"sessionService\").getSessionsCount()",
			"example: runAll app.isMaster()"
	],
};
